...
class CoinGecko {
    ...
    get coins() {
        ...
        return {
            ...
            fetchMarketChartRange: async(coinId, params={}) => {
                //Must have coinId
                if (!Utils.isString(coinId) || Utils.isStringEmpty(coinId)) Utils._WARN_('Invalid parameter', 'coinId must be of type: String and greater than 0 characters.');
                ...
                //If no params.days, set to default: 1
                if (params['from'] == undefined || params['to'] == undefined) Utils._WARN_('Invalid parameters', 'From and To parametrs is required')
                

                const method = 'GET';
                let path = `/${pathPrefix}/${coinId}/market_chart/range`;
    ...
    get exchanges() {
        ...
        return {
             ...
             * @returns {ReturnObject}
             */
            fetchVolumeChart: async (exchangeId, params = {}) => {
              //Must have exchangeId
              if (!Utils.isString(exchangeId) || Utils.isStringEmpty(exchangeId)) Utils._WARN_('Invalid parameter', 'exchangeId must be of type: String and greater than 0 characters.');

              const method = 'GET';
              let path = `/${pathPrefix}/${exchangeId}/volume_chart`;
